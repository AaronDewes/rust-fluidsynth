...
impl Sequencer {
    ...
    pub fn register_client<T: Fn(u32, Event, Sequencer)>(&self, name: &str, callback: T) -> i16 {
        ...
        unsafe {
            ...
            return fluid_sequencer_register_client(
                self.c_fluid_sequencer,
                name_str.as_ptr(),
                Some(event_callback_wrapper::<T>),
                user_data,
            ) as i16;
        }

        extern "C" fn event_callback_wrapper<T>(
            time: c_uint,
    ...
    pub fn get_client_name(&self, index: c_short) -> &str {
        unsafe {
            let raw_name = fluid_sequencer_get_client_name(self.to_raw(), index);
            let slice = CStr::from_ptr(raw_name);
            str::from_utf8(slice.to_bytes()).unwrap()
        }
    }

    pub fn client_is_dest(&self, index: c_short) -> bool {
        unsafe { fluid_sequencer_client_is_dest(self.to_raw(), index) != 0 }
    }
