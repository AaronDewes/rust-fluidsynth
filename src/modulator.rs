...
impl Modulator {
    ...
    pub fn set_source1(&self, src: ModulatorSource, flags: ModulatorFlags) {
        unsafe { fluid_mod_set_source1(self.to_raw(), src as c_int, flags as c_int) }
    }

    pub fn set_source2(&self, src: ModulatorSource, flags: ModulatorFlags) {
        unsafe { fluid_mod_set_source2(self.to_raw(), src as c_int, flags as c_int) }
    }

    pub fn set_dest(&self, dest: GenType) {
        unsafe { fluid_mod_set_dest(self.to_raw(), dest as c_int) }
    }

    pub fn set_amount(&self, amount: f64) {
        unsafe { fluid_mod_set_amount(self.to_raw(), amount as c_double) }
    }

    pub fn get_source1(&self) -> ModulatorSource {
        unsafe { transmute(fluid_mod_get_source1(self.to_raw())) }
    }

    pub fn get_source2(&self) -> ModulatorSource {
        unsafe { transmute(fluid_mod_get_source2(self.to_raw())) }
    }

    pub fn get_flags1(&self) -> ModulatorFlags {
        unsafe { transmute(fluid_mod_get_flags1(self.to_raw())) }
    }

    pub fn get_flags2(&self) -> ModulatorFlags {
        unsafe { transmute(fluid_mod_get_flags2(self.to_raw())) }
    }

    pub fn get_dest(&self) -> GenType {
        unsafe { transmute(fluid_mod_get_dest(self.to_raw())) }
    }

    pub fn get_amount(&self) -> f64 {
        unsafe { fluid_mod_get_amount(self.to_raw()) }
    }

    pub fn test_identity(&self, mod2: &Modulator) -> bool {
        unsafe { fluid_mod_test_identity(self.to_raw(), mod2.to_raw()) != 0 }
    }
